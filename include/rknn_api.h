...
/* Don't invalid output buffer cache.
   ...
   This flags is generally used when the model data buffer is allocated by NPU,
   but is accessed by the CPU,
...
typedef uint64_t rknn_context;
    ...
    */
    ...
    /*
        the information for RKNN_QUERY_DEVICE_MEM_INFO.
    */
    typedef struct _rknn_device_mem_info
    {
        uint32_t total; /* Total available memory size. */
        uint32_t used;  /* Used memory size. */
        uint32_t free;  /* Free memory size. */
    } rknn_device_mem_info;
    ...
    */
    ...
    {
        uint32_t version;         /* the version of extend, must be set to 0. */
        rknn_core_mask core_mask; /* the mask of npu core. */
        const char *model_name;   /* the name of model, for debugging. */
        uint32_t flags;           /* the flags of model. */
    } rknn_init_extend;
    ...
    /*  rknn_init
        ...
        input:
            ...
            rknn_init_extend* extend    the extend information of init, NULL means default.
        return:
            int                         error code.
    */
    ...
    */
    ...
    /*  rknn_mem_sync
        ...
        synchronize the tensor memory buffer.

        input:
            rknn_context ctx            the handle of context.
            rknn_tensor_mem *mem        the pointer of tensor memory information.
            rknn_mem_sync_mode mode     the mode of sync.
        return:
            int                         error code.
    */
    ...
    /*  rknn_create_mem2 (memory allocated inside)
        ...
        Create tensor memory with flags. This API require libdrm support!

        input:
            rknn_context ctx            the handle of context.
            uint64_t size               the size of tensor buffer.
            uint32_t flags              the flags of memory, see the define of RKNN_FLAG_MEMORY_XXX.
        return:
            rknn_tensor_mem*            the pointer of tensor memory information.
    */
    rknn_tensor_mem *rknn_create_mem2(rknn_context ctx, uint64_t size, uint32_t flags);

    /*  rknn_set_core_mask

        set the core mask for npu.
    ...
    */
    ...
    /*  rknn_set_runtime_attr

        set the runtime attribute for npu.

        input:
            rknn_context ctx            the handle of context.
            const char* attr            the attribute string.
        return:
            int                         error code.
    */
    int rknn_set_runtime_attr(rknn_context ctx, const char *attr);
